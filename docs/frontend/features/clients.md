...
3. Trainer dashboard **Need a program** card → `/(trainer)/clients-without-program`: list of active linked clients with **no** `clientProgramAssignments.status='active'`, with **Quick assign program** (`ChooseProgramTemplateSheet` → `AssignToClientsSheet`, `initialClientIds` preset).
4. The "+" header button opens `/(trainer)/add-client` ([linking](./linking.md)).
5. Inside the details screen the trainer can:
...
- Screens
  ...
  - `features/clients/screens/ClientsWithoutProgramScreen.tsx`
- Hooks
  ...
  - `features/clients/hooks/useClientsWithoutActiveProgram.ts`
- Components
  ...
  - `features/clients/components/no-program/ClientNoProgramRow.tsx`
- API
  - `features/clients/api/assignments.api.ts` (workout/program assignment functions)
  - `features/clients/api/clientsWithoutProgram.api.ts` — `anvil_get_trainer_clients_without_active_program`
- Types & utils
...
- Routes
  ...
  - `app/(trainer)/clients-without-program.tsx`
...
- `ClientNoProgramRow` — avatar, name, last check-in pill, quick assign CTA; tap body opens client details.
...
- `useClientsWithoutActiveProgram()` — loads the without-program roster RPC with pull-to-refresh + focus reload.
...
- RPCs:
  ...
  - `anvil_get_trainer_clients_without_active_program()` — rows for `ClientsWithoutProgramScreen`.
...
- Clients list now uses 3 filter pills (`All`, `Active`, `Archived`) with counts and live list filtering.
- Client ordering keeps active links first in the list.
- Individual client cards use brand-aware accent gradients from the active theme and improved hierarchy (identity, target/check-in, assignment state, and actions).
- Client card status pills use distinct colors for active vs archived to improve scanability.
- Screen-level horizontal spacing follows the shared app-wide gutter helper (`getScreenHorizontalPadding`) for consistent edge spacing.
...
2026-05-04 — `ClientsWithoutProgramScreen`, `ClientNoProgramRow`, `useClientsWithoutActiveProgram`, `clientsWithoutProgram.api.ts`, route `/(trainer)/clients-without-program`.
