...
type Props = {
  visible: boolean;
  onClose: () => void;
  onSelectWorkoutTemplate: (workoutId: string, title: string) => void;
};

export function ChooseWorkoutTemplateSheet({
  visible,
  onClose,
  onSelectWorkoutTemplate,
}: Props) {
          ...
          {loading ? (
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <WorkoutCard
                  workout={item}
                  updatedAtLabel={t("library.workoutsList.updatedAt", "Updated")}
                  defaultTitle={t("builder.workoutDetails.defaultTitle", "Untitled workout")}
                  onPress={() => {
                    onSelectWorkoutTemplate(
                      item.id,
                      item.title || t("builder.workoutDetails.defaultTitle", "Untitled workout")
                    );
                    onClose();
                  }}
                />
